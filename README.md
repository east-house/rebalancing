...
포트폴리오 보고서는 시작 보고일·초기 투자금으로 만든 정수 주식 가상계좌를 추적합니다.
최초 매수와 월간 종목 교체·비중 조정은 보고서의 `proposed_execution_date` 종가가
확인될 때 처리하며, 편도 비용 0.10%와 정수 수량을 적용하고 잔액은 현금으로 남깁니다.
총자산·총손익·실현손익·평가손익, 종목별 매수일·평균단가, 거래원장과 일별 계좌를
표시합니다. 새 보고서만 순차 반영하고 이미 저장한 거래는 재실행하지 않습니다.
기존 소수점 보유 기록은 별도 보존하며 자동 변환하지 않습니다. 초기 투자금 변경은
기존 원장을 보관한 후 새 계좌로 시작합니다. 실제 증권사 주문·입출금은 수행하지 않습니다.

운영 보고서를 로컬 코드 화면에서 확인하려면 `npm run dev:reports`를 실행하고
`http://127.0.0.1:5173/portfolio-report`를 엽니다. 이 서버는 최신 운영 보고서 API를
연결하므로 시작 시점의 과거 스냅샷에 고정되지 않습니다.
