...
function feeRate(value: number): string {
...
export default function TradingTestReportPage({
  ...
  const allPositions = useMemo(() => report
    ...
    : [], [report, strategies]);
  const costDescription = report?.transactionCosts
    ? `매수 ${feeRate(report.transactionCosts.buyRate)} / 매도 ${feeRate(report.transactionCosts.sellRate)}`
