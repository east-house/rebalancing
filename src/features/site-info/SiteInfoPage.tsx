  ...
  return (
    <>
      <section>
        ...
        <p>TM Reports는 미국 시장의 흐름과 위험선호, 섹터·테마 리더십, 거시경제 일정과 관련 뉴스를 한 화면에서 해석할 수 있도록 정리하는 독립 정보 서비스입니다. 리밸런싱과 ETF 연구 도구는 이용자가 직접 입력하거나 선택한 조건을 브라우저에서 계산해 비교할 수 있게 돕습니다.</p>
      </section>
      <section>
        <h2>콘텐츠 작성 원칙</h2>
        <ul>
      ...
      <section>
        <h2>업데이트와 한계</h2>
        <p>시장 리포트는 매일 한국시간 오전 7시 30분에 실행 당일 날짜로 생성되며, 분석에 사용한 미국 거래일은 별도로 표시합니다. 주말과 미국 휴장일에는 가장 최근에 확인된 거래일 자료가 사용될 수 있습니다. 가격과 경제지표는 제공처의 갱신 시점, 휴장, 네트워크 상태에 따라 지연되거나 누락될 수 있습니다.</p>
      </section>
      <section>
        <h2>독립성</h2>
  ...
  return (
    <>
      ...
      <section>
        ...
        <p>이 방침은 TM Reports 웹사이트 이용 과정에서 처리되는 정보에 적용됩니다. 서비스는 회원가입을 받지 않으며, 이용자가 리밸런싱 화면에 입력한 보유 종목·수량·평단가·목표 비중·투자금과 계산 이력은 원칙적으로 해당 브라우저의 로컬 저장소에만 보관됩니다.</p>
      </section>
      <section>
        <h2>2. 처리될 수 있는 정보</h2>
        <ul>
          <li><strong>브라우저 저장 정보:</strong> 포트폴리오 입력값, 화면 설정과 계산 이력. 사이트 서버로 전송하지 않으며 이용자가 화면의 삭제 기능이나 브라우저 설정으로 지울 수 있습니다.</li>
