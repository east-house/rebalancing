...
- 원본은 미국 월말 신호, 현재는 기존의 매월 첫 한국 평일 보고서에서 판단한다.
  이는 날짜 표기의 차이이며 2025~2027년 36개월 비교에서 실제 판단 미국 거래일과
  다음 체결 거래일이 일치한다. 처음에는 날짜 차이 가능성을 보고했으나 대조 후
  정정했다. 최초 계좌 시작은 사용자가 고른 보고일이므로 월중에도 가능하다.
  월간 날짜 선택·휴장일·게시 스케줄은 이번 작업에서 수정하지 않았다.
...
- Python 파이프라인 117개 테스트 통과(36개월 월간 판단·체결일 일치 검사 포함).
